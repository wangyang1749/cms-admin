    ...
    <div>
        <div id="app-menu">
                ...
                <div class="weixin-bd">
                    ...
                    <ul class="weixin-menu" id="weixin-menu">
                        <li v-for="(btn,i) in menu.buttons" :key="i" class="menu-item" 
                            :class="{'current': selectedMenuIndex === i && selectedMenuLevel == 1 }" @click="selectMenu(i)">
                            <div class="menu-item-title">
                                <span>{{ btn.name }}</span>
                            </div>
                            <ul class="weixin-sub-menu">
                                <li v-for="(sub,i2) in btn.subButtons" :key="i2" class="menu-sub-item" 
                                    :class="{'current' :selectedMenuIndex === i && selectedSubMenuIndex === i2 && selectedMenuLevel == 2,
                                    'on-drag-over': onDragOverMenu == (i + '_' + i2)}" draggable="true"
            ...
            <div class="weixin-menu-detail" v-if="selectedMenuLevel>0">
                <wx-Editor :button="selectedButton" :selectedMenuLevel="selectedMenuLevel" @delMenu="delMenu"></wx-Editor>
            </div>
        </div>
...
<script>
import wxmanage from "@/api/wxmanage.js";
import wxEditor from "./wxEditor.vue";
export default {
    components: {
        wxEditor
    },
    ...
    methods: {
        ...
        updateWxMenu() {
            wxmanage.updateMenu(this.menu).then((res) => {
